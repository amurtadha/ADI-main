...
class Instructor:
    ...
    def _train(self,model,optimizer,criterion_y,criterion_d,train_data_loader, val_data_loader, test_data_loader, t_total):
        ...
        for epoch in range(self.opt.num_epoch):
            ...
            with torch.no_grad():
                ...
                # logger.info(f'precisions: {val_precisions}')
                # logger.info(f'recalls: {val_recalls}')
                # logger.info(f'f1s: {val_f1s}')
    ...
    def run(self):
        ...
        outputs_path = path.replace('models/', 'outputs/')
        if not os.path.exists(outputs_path):
            os.mkdir(outputs_path)

        with open(f'{outputs_path}/aadi_{self.opt.dataset}_stats.csv', 'w') as f:
        ...
        np.save(f"{outputs_path}/aadi_{self.opt.dataset}_reps.npy", reps)
